...
app.post("/api/events", async (req, res) => {
    ...
    if (filters?.length > 0) {
        ...
        const validEventTypes = ["Workshop", "Hackathon", "Competitions", "Other"];
...
app.post('/api/notices/upload', upload.single('image'), async (req, res) => {
    try {
        ...
        const { eventName,eventDate, eventTime, eventVenue, requestedBy } = req.body;
        ...
        const existingEvent = await Event.findOne({ eventDate, eventVenue,eventName });
